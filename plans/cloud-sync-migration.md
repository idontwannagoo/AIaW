...
### 下一步：Stage 4.5 Step 1 ImportJob 模型

批次-4e messages 已落地（dialogId mandatory + ref envelope spill + scoped pull + dialog/workspace cascade + 流式节流三触发）。Stage 4 主体 5 张表全部迁完。下一步：Stage 4.5 Step 1 — ImportJob 模型（`POST /api/v1/import-jobs` + alembic migration + partial unique index 保证每用户同一时刻只 1 个 active job）；详见 Stage 4.5 段「Step 1」。

### 未启动（按依赖顺序）

- **Stage 4.5** 服务端 Import Job + bootstrap ⏳ ← **下一步 / 可让老用户用的物理分水岭**
