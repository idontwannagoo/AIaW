...
### 下一步：Stage 4.5 / Step 4 — Phase C messages 文字写入

Stage 4.5 / Step 3 已落地（PHASE_B_TABLES 7 表依赖序 LWW UPSERT + 10 张 server-routed 表新增 imported_from_job_id/imported_at 两列 + 索引 + FK ON DELETE SET NULL + DELETE cascade 共享 cascade_version 软删 + per-row WS delete event；14 case 全绿 + 故障注入 3 轮均 catch + 1 轮 dev 自带 `text` import 漏报 catch）。下一步 Step 4 实现 `import_worker.py::run_phase_c`：流式遍历 `messages.ndjson`，每 500 行一批 `INSERT ON CONFLICT DO UPDATE`，attachment 字段保留 inline base64 进 PG（messages 行加 `_pending_blob_extraction BOOLEAN DEFAULT FALSE` 列），FK 约束失败的孤儿 row 进 `import_jobs.dead_letter`，状态转 phase_d。详见 Stage 4.5 段「Step 4」。

### 未启动（按依赖顺序）

- **Stage 4.5** 服务端 Import Job + bootstrap ⏳ ← **进行中（Step 1-3 ✅ / Steps 4-8 待办）**
