...
- **服务 2（指向 `new-deploy`）**：公网域名 `https://p01--new-aiaw--hqdb2bsvdnbt.code.run`，独立 Postgres
  - 前端 `.env.docker` 三档全开：`BACKEND_DATA_API_URL=<self>` + `BACKEND_AUTH=true` + `BACKEND_DATA_TABLES=providers,reactives,assistants,installedPlugins,avatarImages,workspaces,dialogs,items,artifacts` + `REALTIME_TRANSPORT=auto` + `DEXIE_DB_URL=`（留空）
  ...
  - 实测：`/api/v1/health` `{status:"ok",db:"ok"}`、`/api/v1/auth/me` 401、`/api/v1/providers` 401、`/api/v1/reactives` 401、`/api/v1/assistants` 401、`/api/v1/avatar-images` 401、`/api/v1/installed-plugins` 401、`/api/v1/workspaces` 401、`/api/v1/dialogs` 401、`/api/v1/items` 401、`/api/v1/artifacts` 401、`/api/v1/auth/register` 422
  - **当前能用 / 不能用**：providers + reactives + assistants + installedPlugins + avatarImages + workspaces + dialogs + items + artifacts 跨设备同步可用；dialogs / items / artifacts 已支持 scoped pull（按 `?workspaceId=` / `?dialogId=` lazy 拉，打开单 workspace / dialog 不再触发全表 fetch）；workspace 删除时 dialogs + items + artifacts 走 server 端真级联（同事务 tombstone + 各发 WS event，共用 cascade_version；artifacts 直接按 workspace_id 一跳，无 dialog hop）；items.contentBuffer ≥ 64KB / artifacts.versions JSON ≥ 64KB 自动走对象存储 ref + 跨 tab `materializeAttachment` 字节级一致；剩 1 张表（messages）仍只在本地 IndexedDB，workspace 删除时由前端 `stores/workspaces.ts::deleteItem` 顺序 await 清；老用户旧数据无法导入（ImportJob 未做）。**仅适合自己 dev preview，不要导入真实数据，也不要邀请他人**

### 下一步：批次-4e messages

批次-4d artifacts 已落地（单 scope `workspaceId` + ref 大版本载体 + scoped pull + workspace cascade），剩余 1 张未迁表 `messages`。下一步：批次-4e `messages`（依赖硬前置 1 cursor 分页 + 硬前置 2 对象存储 + 硬前置 3 scoped pull + 流式节流 200ms+1KB+sentence 三触发，`?dialogId=` mandatory）。

### 未启动（按依赖顺序）

- **Stage 4 主体** 批次-4e `messages`（含流式节流 200ms+1KB+sentence 三触发）⏳ ← **下一步**
