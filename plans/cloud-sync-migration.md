...
### 下一步：Stage 4.5 / Step 6 — WS 进度推送 + 状态持久化（集成验证）

Stage 4.5 / Step 5 已落地（`run_phase_d` + `_phase_d_process_row` + `_phase_d_upload_one` 三层闭包：扫 `messages WHERE user_id=? AND _pending_blob_extraction = TRUE`（partial index）+ inline base64 解码 + actual_size 阈值 < 64KB 留 inline / ≥ 64KB 上传 BlobStore + 改写 envelope 为 `{type:'ref', url, sha256, size, content_type}` + 4 路 asyncio.Semaphore 包**单 attachment** + 重试 3 次（exp backoff 1/2/4s）+ 单 attachment 4 attempt 全失败入 dead_letter（envelope 留 inline，row 仍可读）+ 单 row 同 SessionLocal tx clear `_pending_blob_extraction` + bump version/updated_at + final progress publish + tmp dir 清理 + raw_object_key 共享 sha256 检查后 BlobStore.delete；status='phase_d' → 'done'。13 case 全绿 + 故障注入 3 轮全 catch）。下一步 Step 6 把 `import_jobs` 注册进 `stream.py::TABLE_MODELS` 让 client 能 WS subscribe + 把客户端写 `PUT /api/v1/import_jobs/<id>` 405 兜底（worker-only 写）+ 真 WS 帧抓取替代当前 indirect-PG-poll 进度证；前提：Step 5 现在的 `_publish_phase_d_progress` 已经在 publish 进度，broker 端就位，只差 routed-table 注册和客户端订阅契约。

### 未启动（按依赖顺序）

- **Stage 4.5** 服务端 Import Job + bootstrap ⏳ ← **进行中（Step 1-5 ✅ / Steps 6-8 待办）**
