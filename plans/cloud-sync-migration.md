...
- **服务 2（指向 `new-deploy`）**：公网域名 `https://p01--new-aiaw--hqdb2bsvdnbt.code.run`，独立 Postgres
  ...
  - **当前能用 / 不能用**：providers + reactives + assistants + installedPlugins + avatarImages + workspaces + dialogs + items + artifacts + messages 跨设备同步可用（**Stage 4 主体收尾，0 张表未迁**）；dialogs / items / artifacts / messages 已支持 scoped pull（按 `?workspaceId=` / `?dialogId=` lazy 拉，messages 是 mandatory `?dialogId=`，打开单 workspace / dialog 不再触发全表 fetch）；workspace 删除时 dialogs + items + artifacts + messages 走 server 端真级联（同事务 tombstone + 各发 WS event，共用 cascade_version；artifacts 直接按 workspace_id 一跳，messages 经 dialogs 二跳）；dialog 删除时 messages 同事务级联 tombstone + WS event 链；items.contentBuffer ≥ 64KB / artifacts.versions JSON ≥ 64KB / messages 整体 envelope JSON ≥ 64KB 自动走对象存储 ref + 跨 tab `materializeAttachment` 字节级一致；messages 跨 tab 流式同步可用（inline envelope + 200ms+1KB+sentence 三触发节流，5min ~3000 字流式回复期间 broker 入队 ≤ 25 events 内）；老用户旧数据无法导入（ImportJob 未做）。**Stage 4 主体收尾，可邀请少量内圈测试，但 ImportJob 未做老用户导入仍待 Stage 4.5；不要导入真实大量数据**

### 下一步：Stage 4.5 / Step 8 — `GET /api/v1/bootstrap` endpoint + 首屏路由 guard

Stage 4.5 / Step 7 已落地（`src/data/import-client.ts` ~360 行 multipart upload helper + Promise pool 4 并发 + 5MB 切片 + localStorage cursor 续传 + retry 3 + AbortSignal + `subscribeImportStatus` 包 realtime；`src/composables/import-job.ts` 全局 active-job 单例 ref + auth-watch 自动 fetch + subscribe + terminal 自动清理；`src/boot/import-job.ts` app 启动安装 watcher；`ImportDataDialog.vue` 整段重写：选文件→create→uploadParts (4 路并发 + 5 tick 滑窗 ETA)→complete + cancel 走 AbortController；`AccountPage.vue` 迁移状态卡片：phase label + 双线进度条按 phase 切分母 + cancel/dismiss 互斥 + 后台通知文案；i18n 三语 16 keys；`expose-debug.ts` 暴露 `window.__importClient__` 全 API + `cursorKey/readCursor` 测试用 helper；`tests/env/.env.test.import-job` 新 profile（与 realtime-ws 同 flag 集 + 独立 build cache slot port 9015）+ `playwright.config.ts` / `run-playwright.sh` / `backend-start.sh` CORS 全配套；`make_medium_export` fixture builder 新增。6 case 全绿 + 故障注入 3 轮全 catch（cursor 写入 / resume 路径 / subscribe 过滤）。下一步 Step 8 写后端 `src-backend/data/routers/bootstrap.py` `GET /api/v1/bootstrap` 一次返所有小表全量 + per-dialog 最近 50 条 messages 摘要（不含 items / artifacts，2026-05-03 修订生效）+ 1MB 单响应硬上限 + `Cache-Control: private, max-age=10` + 前端 `src/router/index.ts` 加 router guard 登录后 `await GET /api/v1/bootstrap` 写 IDB → 放路由进 MainLayout + 2s timeout fallback 走老路径 + banner 提示，落 7 case（5 backend + 2 e2e）覆盖 size 限额 + 截断 + active import 期间正确返部分数据 + 1.5s 内首屏可见 + timeout fallback。

### 未启动（按依赖顺序）

- **Stage 4.5** 服务端 Import Job + bootstrap ⏳ ← **进行中（Step 1-7 ✅ / Step 8 待办）**
