...
### 下一步：Stage 4.5 / Step 3 — Phase B 结构表写入

Stage 4.5 / Step 2 已落地（5 public endpoint + LocalFs `_internal` PUT shim + 4 BlobStore multipart 抽象方法 + LocalFs 完整模拟实现 + worker queued→parsing→phase_b 自动推进 + 12 case 全绿 + 故障注入 3 轮均 catch）。下一步 Step 3 实现 `import_worker.py::run_phase_b`：从 Phase A 输出的 NDJSON 按依赖顺序读小表（providers / assistants / plugins / reactives / avatarImages / workspaces / dialogs metadata），按表 `INSERT ... ON CONFLICT (id) DO UPDATE WHERE existing.updated_at < incoming.updated_at` LWW 写入，每行附 `imported_from_job_id` + `imported_at` 两列（所有 server-routed 表 alembic 加这两列；Stage 1-4 已落地的表按需 backfill NULL），完成后状态转 phase_c 并发 WS event。详见 Stage 4.5 段「Step 3」。

### 未启动（按依赖顺序）

- **Stage 4.5** 服务端 Import Job + bootstrap ⏳ ← **进行中（Step 1 ✅ / Step 2 ✅ / Steps 3–8 待办）**
