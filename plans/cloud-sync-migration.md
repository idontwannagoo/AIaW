...
### 下一步：Stage 4.5 / Step 5 — Phase D attachments → 对象存储

Stage 4.5 / Step 4 已落地（`run_phase_c` 流式 messages.ndjson + 500 行/批 LWW UPSERT + `_pending_blob_extraction` 双 prong 判定（size ≥ 64KB OR 含 envelope，partial b-tree 索引）+ orphan FK 失败入 `dead_letter` JSONB 数组 array-concat 不丢历史 + per-batch 进度事件 + status → phase_d；14 case 全绿 + 故障注入 3 轮全 catch）。下一步 Step 5 实现 `import_worker.py::run_phase_d`：扫 `messages WHERE _pending_blob_extraction=TRUE AND user_id=?` → 解码 inline base64 → ≥ 64KB 上传 BlobStore + 改写 row 为 `{type:'ref', url, sha256, size, content_type}` + 4 路并发 + 重试 3 次（指数退避 1s/2s/4s）+ 单条失败超重试入 dead_letter + 全部完成清 `_pending_blob_extraction=FALSE` → status='done' → 清 `/tmp/import-<job_id>/` + 对象存储 raw upload（TTL 7 天）。详见 Stage 4.5 段「Step 5」。

### 未启动（按依赖顺序）

- **Stage 4.5** 服务端 Import Job + bootstrap ⏳ ← **进行中（Step 1-4 ✅ / Steps 5-8 待办）**
