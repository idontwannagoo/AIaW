...
### 下一步：Stage 4.5 / Step 7 — 前端 ImportDataDialog 重写 + 上传切片 helper

Stage 4.5 / Step 6 已落地（`stream.py` / `sse.py` `TABLE_MODELS` + `SERIALIZERS` 加 import_jobs 入口 + `_serialize_import_job(j)` 复用 `ImportJob._envelope()` server-routed 契约 + `routers/imports.py` 加 405 reject 路由 `/api/v1/import_jobs/{job_id}` PUT/PATCH/POST/DELETE 全 405 + body 'read-only' + `Allow: ''` header；GET 同路径因其他方法已注册也返 405，唯一读入口仍是 `GET /api/v1/import/jobs/{id}`（带斜线 path）。8 case 全绿 + 故障注入 3 轮全 catch（routed-table 注册 / 405 路由 / broker user 隔离）。Step 6 是集成验证 step，无新 backend 业务逻辑——Step 5 已经在 phase B/C/D 推 progress，Step 6 只是把 import_jobs 注册到 WS/SSE 两个 transport 让 client 能 subscribe + 405 兜底防止误写。下一步 Step 7 写前端 `src/data/import-client.ts`（multipart 切片 + 进度 + 续传 + cancel + subscribeImportStatus）+ `ImportDataDialog.vue` 重写 + `AccountPage.vue` 加迁移状态卡片 + 应用启动时 `GET /api/v1/import/jobs?status=active` 自动挂 subscribe，落 e2e profile `import-job` 6 case（fixture small / medium 端到端含 phase B/C/D 完成可见性 + 关 tab 重开见进度 + 网络中断续传 slow + cancel + attachment 渲染）。

### 未启动（按依赖顺序）

- **Stage 4.5** 服务端 Import Job + bootstrap ⏳ ← **进行中（Step 1-6 ✅ / Steps 7-8 待办）**
