...
### 下一步：Stage 4.5 / Step 2 — multipart 上传 5 endpoint

Stage 4.5 / Step 1 已落地（ImportJob 表 + worker 单进程主循环 + Phase A ijson 流式解析 + partial unique index + 8 case 全绿 + 故障注入 2 轮均 catch）。下一步 Step 2 实现 `POST/GET/DELETE /api/v1/import/jobs[/{id}/...]` 5 个 endpoint：客户端上传 raw `aiaw_user_db.json` 走 multipart（创建 job + presigned PUT URL + complete 触发 worker + GET 状态 + 取消 + 跨用户 404 隔离），并在 `BlobStore` 抽象上加 4 个 multipart 方法（local-fs / s3 两实现）。详见 Stage 4.5 段「Step 2」。

### 未启动（按依赖顺序）

- **Stage 4.5** 服务端 Import Job + bootstrap ⏳ ← **进行中（Step 1 ✅ / Steps 2–8 待办）**
